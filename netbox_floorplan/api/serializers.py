...
class FloorplanSerializer(NetBoxModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'url', 'site', 'location', 'background_image',
                  'scale', 'tags', 'custom_fields', 'created', 'last_updated', 'canvas', 'measurement_unit']


class FloorplanObjectSerializer(NetBoxModelSerializer):
    url = serializers.HyperlinkedIdentityField(
        view_name='plugins-api:netbox_floorplan-api:floorplanobject-detail')

    class Meta:
        model = FloorplanObject
        fields = ['id', 'url', 'rack', 'location', 'floorplan', 'x_coordinate',
                  'y_coordinate', 'tags', 'custom_fields', 'created', 'last_updated', 'rotation']
